...
def show_experiment_information(model, x, st_observation_list, st_prediction_list, xt_prediction_list, position, fname):
    ...
    # observation phase
    t = model.observe_dim - 1
    print(f'observation at {t} step')
    ...
    plt.savefig(fname + f"obs_{t}.png")
    plt.close()

    # predition phase
    '''
    for t in range(model.total_dim - model.observe_dim):
        print(f'prediction at {t} step')
        ...
        plt.plot(position[sa1mple_id, , t + model.observe_dim], position[sample_id, 0, t + model.observe_dim], 'bs')
        ...
        # plt.pause(0.01)
        plt.savefig(f"./results/{exp}/prd_{t}.png")
